...
const StatsVendedor = ({ totalPubs }) => {
  ...
  useEffect(() => {
    api.get("/oferta/mis-ofertas-recibidas")
      .then((res) => {
        ...
        ofertas.forEach((o) => {
          const key = new Date(o.fecha || o.createdAt).toLocaleString("es", { month: "short" });
          if (key in meses) meses[key]++;
        });
        setOfertasPorMes(Object.entries(meses).map(([mes, cantidad]) => ({ mes, cantidad })));
      })
      .catch(() => {
        const meses = [];
        const now = new Date();
        for (let i = 5; i >= 0; i--) {
          const d = new Date(now.getFullYear(), now.getMonth() - i, 1);
          meses.push({ mes: d.toLocaleString("es", { month: "short" }), cantidad: 0 });
        }
  ...
  return (
    <div className="stats-row stats-row-2">
      ...
      {esPlus ? (
        <div className="stat-card">
          ...
          <div className="stat-value">
            {totalPubs}<span className="unit">/ {LIMIT_PLUS}</span>
          </div>
          <div style={{ marginTop: ".5rem" }}>
            <div style={{ height: 4, borderRadius: 2, background: "var(--border-subtle)", overflow: "hidden" }}>
              <div style={{
                height: "100%", width: `${usoPct}%`,
                background: usoPct >= 100 ? "#e05252" : "var(--gold)",
                borderRadius: 2, transition: "width .4s",
              }} />
            </div>
            <div style={{ fontSize: ".65rem", color: "var(--text-muted)", marginTop: ".3rem" }}>
      ...
      <div className="stat-card">
        <div className="stat-label">Actividad <em>mensual</em></div>
          ...
          <Bar
            data={{
              ...
              datasets: [{
                ...
                backgroundColor: ofertasPorMes.map((_, i) =>
                  i === ofertasPorMes.length - 1 ? "rgba(212,175,55,.9)" : "rgba(212,175,55,.3)"
                ),
                borderRadius: 3,
                borderSkipped: false,
              }],
            ...
            options={{
              ...
              plugins: {
                ...
                tooltip: { callbacks: { label: (ctx) => ` ${ctx.raw} ofertas` } },
              },
              scales: {
                x: { ticks: { color: "#666", font: { size: 9 } }, grid: { display: false }, border: { display: false } },
                y: { display: false, grid: { display: false } },
              },
            }}
          />
        </div>
        <div style={{ fontSize: ".65rem", color: "var(--text-muted)", marginTop: ".2rem" }}>Ofertas por mes</div>
      </div>
    </div>
  );
};
